...
from pipeline.context.scip.proto.scip_pb2 import Document, Occurrence as ScipOccurrence, SymbolInformation
...
class Symbol:
    ...
    def from_symbol_information(
        ...
        if kind == "UnspecifiedKind":
            kind = _kind_from_symbol_id(symbol_info.symbol)
...
class Occurrence:
    ...
    def from_occurrence(
        ...
        file_path = document.relative_path.replace("\\", "/")
        ...
        if is_definition and occurrence.enclosing_range:
            start_line, start_character, end_line, end_character = _normalize_range(occurrence.enclosing_range)
        else:
...
def _kind_from_symbol_id(symbol_id: str) -> str:
    if not symbol_id or symbol_id.startswith("local "):
        return "Local"

    last = symbol_id[-1]
    if last == "#":
        return "Class"
    if last == "." and len(symbol_id) >= 2 and symbol_id[-2] == ")":
        return "Method"
    if last == ".":
        return "Term"
    if last == ")":
...
def _find_nearest_enclosing_definition(
    ...
    for definition in definitions:
        ...
        raw_range = definition.enclosing_range if definition.enclosing_range else definition.range
        definition_range = _normalize_range(raw_range)
...
class Cluster:
    ...
    subgraph: dict[str, dict[str, int]] | None = None
